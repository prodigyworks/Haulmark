...
function showMap(id) {
	callAjax(
			"finddata.php", 
			{ 
				sql: "SELECT B.id, A.startvisittype, A.endvisittype, A.fromplace, A.toplace, B.place " + 
					 "FROM hallmark_booking A " +
					 "LEFT OUTER JOIN hallmark_bookingleg B " + 
					 "ON B.bookingid = A.id " +
					 "WHERE A.id = " + id + " " +
					 "ORDER BY B.id"
			},
			function(data) {
				if (data.length > 0) {
					...
					initializeMap2(node.fromplace, node.toplace, waypoints, 0);
...
function addPoint(hideDeparture) {
    ...
    if (hideDeparture) {
    	html = "<div id='container_" + counter + "' class='pointcontainer bookingjourneys' index='" + counter + "' style='padding-top:3px'>\n" +
		   ...
		   "	<div class='bubble' title='Required field'></div>\n" +
		   "	<img title='Collection' id='point_" + counter  + "_imgcollect' src='images/booking_pointcollect_red.png' associate='point_" + counter  + "_visittype' onclick='collectPoint(this)' class='booking_pointcollect hoverimage' ></img>" +
		   "	<img title='Delivery' id='point_" + counter  + "_imgdelivery' src='images/booking_pointdelivery.png' associate='point_" + counter  + "_visittype' onclick='deliverPoint(this)' class='booking_pointdelivery hoverimage' ></img>" +
		   "	<img src='images/minus.gif' class='pointimage' onclick='removePoint(this)'></img>" +
		   "	<input id='point_" + counter  + "_visittype' class='visittype' type='hidden' name='point_" + counter + "_visittype' value='C' >\n" +
    	...
    	html = "<div id='container_" + counter + "' class='pointcontainer bookingjourneys' index='" + counter + "' style='padding-top:3px'>\n" +
		   ...
		   "	<img title='Delivery' id='point_" + counter  + "_imgdelivery' src='images/booking_pointdelivery.png' associate='point_" + counter  + "_visittype' onclick='deliverPoint(this)' class='booking_pointdelivery hoverimage' ></img>" +
		   "	<img src='images/minus.gif' class='pointimage' onclick='removePoint(this)'></img>" +
		   "	<input id='point_" + counter  + "_visittype' type='hidden' class='visittype' name='point_" + counter + "_visittype' value='C' >\n" +
		   "	<input id='point_" + counter  + "_lng' type='hidden' name='point_" + counter + "_lng' value=''>\n" +
		   "	<input id='point_" + counter  + "_lat' type='hidden' name='point_" + counter + "_lat' value=''>\n" +
		   "</div>";
...
function collectPoint(node) {
	var data = $("#" + $(node).attr("associate"));
	
	if ($(node).attr("src") == "images/booking_pointcollect_red.png") {
		$(node).attr("src", "images/booking_pointcollect.png");
		...
		$(node).attr("src", "images/booking_pointcollect_red.png");
	}
}

function deliverPoint(node) {
	var data = $("#" + $(node).attr("associate"));
	
	if ($(node).attr("src") == "images/booking_pointdelivery_red.png") {
		$(node).attr("src", "images/booking_pointdelivery.png");
		...
		$(node).attr("src", "images/booking_pointdelivery_red.png");
...
function startPoint(node) {
	...
	if ($(node).attr("src") == "images/booking_pointstart_red.png") {
		$(node).attr("src", "images/booking_pointstart.png");
		...
		$(".booking_pointstart").attr("src", "images/booking_pointstart.png");
		$(".startutlisation").each(
				function() {
		...
		$(node).attr("src", "images/booking_pointstart_red.png");
...
function endPoint(node) {
	...
	if ($(node).attr("src") == "images/booking_pointend_red.png") {
		$(node).attr("src", "images/booking_pointend.png");
		...
		$(".booking_pointend").attr("src", "images/booking_pointend.png");
		$(".startutlisation").each(
				function() {
					if ($(this).val() == "B" || $(this).val() == "S") {
						$(this).val("S");
		...
		$(node).attr("src", "images/booking_pointend_red.png");
...
function loadLegs(id) {
	callAjax(
			"finddata.php", 
			{ 
				sql: "SELECT B.id, A.fromplace, A.fromplace_ref, A.fromplace_phone,  A.toplace, A.toplace_ref, " +
					 "A.toplace_phone, B.visittype, B.place, B.place_lng, place_lat, B.reference, B.phone, " +
			...
			function(data) {
				if (data.length > 0) {
					for (var i = 1; i <= data.length; i++) {
						...
						$("#point_" + i + "_visittype").val(node.visittype);

						if (node.visittype == "C" || node.visittype == "B") {
							$("#point_" + i + "_imgcollect").attr("src", "images/booking_pointcollect_red.png");

						} else {
							$("#point_" + i + "_imgcollect").attr("src", "images/booking_pointcollect.png");
						}
						
						if (node.visittype == "D" || node.visittype == "B") {
							$("#point_" + i + "_imgdelivery").attr("src", "images/booking_pointdelivery_red.png");
							
						} else {
							$("#point_" + i + "_imgdelivery").attr("src", "images/booking_pointdelivery.png");
						}
					}
				}
			},
...
function vehicletypeid_onchange() {
	...
	$.ajax({
			...
			success: function(json) {
				$("#pallets").attr("required", json.groupage);
				$("#vehicleid").html(json.data);
			}
		});
}
...
function vehicleid_onchange() {
	...
	callAjax(
			"finddata.php", 
			{ 
				sql: "SELECT A.vehicletypeid, A.usualtrailerid, A.usualdriverid, A.registration, B.groupage " +
					 "FROM hallmark_vehicle A " +
					 "LEFT OUTER JOIN hallmark_vehicletype B " +
...
function loadAddress(customerid) {
	callAjax(
			...
			{ 
				sql: "SELECT " + 
					 "A.street, A.address2, A.town, A.city, A.county, A.postcode, " + 
					 "A.telephone, A.telephone2, A.fax AS mobile " +
